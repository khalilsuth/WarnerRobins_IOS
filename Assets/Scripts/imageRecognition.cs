...
public class ImageRecognition : MonoBehaviour
{
    ...
    private void OnTrackedImagesChanged(ARTrackedImagesChangedEventArgs e)
    {
        ...
        {
            UnityEngine.Debug.Log($"Tracked image detected: {trackedImage.referenceImage.name} with size: {trackedImage.size}");
        }
    ...
    private void UpdateTrackedImages(IEnumerable<ARTrackedImage> trackedImages)
    {
        ...
        {
            GameObject.FindWithTag(ReferenceImageName).SetActive(true);
